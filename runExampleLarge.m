...
fprintf('\nA is large. This may take a few minutes...\n');

%% Call the solver

% A is large so we use the 'fast' profile.
[x_sol, sol] = sparse_approx_gsm_v1_22(A, y, k, 'profile', 'fast');

% Alternative examples:
% =====================

% Use the thorough profile instead of the normal one:
% [x_sol, sol] = sparse_approx_gsm_v1_22(A, y, k, 'profile', 'thorough');

% Use power 1 for the residual instead of 2. This requires Mosek or Yalmip.
% [x_sol, sol] = sparse_approx_gsm_v1_22(A, y, k, 'residualPower', 1);

% Use the thorough profile but disable postprocessing by OMP:
% [x_sol, sol] = sparse_approx_gsm_v1_22(A, y, k, 'profile', 'thorough', 'postprocess_by_omp', false);

% An alternative way to pass parameters:
% params = {'profile', 'thorough', 'postprocess_by_omp', false};
% [x_sol, sol] = sparse_approx_gsm_v1_22(A, y, k, params);
