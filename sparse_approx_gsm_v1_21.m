...
version = '1.21';  version_date = '23-Jul-2020';
...
qprintln(vl,1,'Weighted l1 solver: %s', (params.solver));
qprintln(vl,1,'');
...
qprintln(vl,2,'Running with %d %s-values, %.3g <= %s_rel <= %.3g', numel(lambda_vals), uLambda,  min(lambda_vals), uLambda, max(lambda_vals));
qprintln(vl,2);
...
while ~isempty(i_next) && (i_next <= numel(db))
    ...
    elseif reached_consecutive_sparsity
        ...
        sol.stop_message = sprintf('Obtained minimizer of F_lambda(x) was k-sparse for %d consecutive values of lambda.', params.nLambdas_sparse_x_to_stop);
    ...
    if tElapsed_curr < 100
        tLambdaRound = round(100*tElapsed_curr)/10;
    else
    ...
    if i == 1
        qprintln(vl, 2, ' %s  %s | %s | %s | %s | %s | %s\n', ...
            ...
            pad_str('nGammas', lengths.nGammas), ...
...
defaultParams.nLambdaVals = chooseByKeyStr(profile, ...
    'fast', 3, 'normal', 10, 'thorough', 31, 'ultra', 43);
